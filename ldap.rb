...
module LdapServer
  ...
  def handle_search_request pdu
    unless @authenticated
      # NOTE, early exit.
      send_ldap_response 5, pdu[0].to_i, 57, "", "Who did you say you were?"
      return
    end
    ...
    if filters.length == 0
      # NOTE, early exit.
      send_ldap_response 5, pdu[0].to_i, 63, "", "No filter specified"
    end
...
if __FILE__ == $0
  ...
  EventMachine.run do
    ...
    EventMachine.add_periodic_timer 90, proc {$logger.info "heartbeat"}
  end
end
